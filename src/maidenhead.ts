...
const WGS84ToMaidenhead = (coord: CoordinateLike): GridLocator => {
  if (typeof coord !== 'object' && !Array.isArray(coord)) throw new Error("invalid coord. it should be [number, number] or {lat: number, lon: number}")
  ...
  if (lon < -180 || lon > 180) throw new Error("invalid lon. it should be between -180 and 180")
  const latIndex = Math.floor((lat + 90) / 10)
  const lonIndex = Math.floor((lon + 180) / 20)
  const latRemainder = Math.floor(lat + 90) % 10
  const lonRemainder = Math.floor(Math.floor(lon + 180) % 20 / 2)
  const latSubIndex = Math.floor((lat - Math.floor(lat)) * 60 / 2.5)
  const lonSubIndex = Math.floor((lon - Math.floor(lon / 2) * 2) * 60 / 5)

  return `${i2l(lonIndex)}${i2l(latIndex)}${lonRemainder}${latRemainder}${(i2l(lonSubIndex) + i2l(latSubIndex)).toLowerCase()}`
}
...
const maidenheadToWGS84 = (gridLocator: GridLocator): WGS84 => {
  ...
  const lonIndex = l2i(gridLocator[0])
  const latIndex = l2i(gridLocator[1])
  const lonRemainder = parseInt(gridLocator[2])
  const latRemainder = parseInt(gridLocator[3])
  const lonSubIndex = l2i(gridLocator[4].toUpperCase())
  const latSubIndex = l2i(gridLocator[5].toUpperCase())
  const lon = lonIndex * 20 - 180 + lonRemainder * 2 + lonSubIndex * 5 / 60
 ...
 */
const maidenheadToBoundingBox = (gridLocator: GridLocator): [LatLon, LatLon] => {
  if (!validateGridLocator(gridLocator)) throw new Error("invalid gridLocator")
  let lon = l2i(gridLocator[0]) * 20 + parseInt(gridLocator[2]) * 2 + l2i(gridLocator[4]) * 5 / 60 - 180;
